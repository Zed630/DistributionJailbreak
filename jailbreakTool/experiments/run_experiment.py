...
from jailbreakTool.modules.defenders import defender
...
def run_experiment(config_path: str):
    ...
    defenders, provide_Defender = {}, ""
    if 'defender_config' in config and config['defender_config']["load_defenders"]:
        defender_info = config["defender_config"]["defend_config"]
        provide_Defender = defender_info["defender_cls"]
        defender = register.get_registered("defender", defender_info["defender_cls"])
        defenders[defender_info["defender_cls"]] = defender(config = defender_info)
        logger.info(f"加载 defender - {defender_info['defender_cls']} 完成")
    ...
    for attacker_cls, attacker in attackers.items():
        ...
        for harmful_idx, harmful_behavior in enumerate(data_loaders['benchmark'].test_harmful_request):
            ...
            jailbreak_result = attacker.case_attack(
                ...
                defender=defenders[provide_Defender] if defenders else None).to_dict()
